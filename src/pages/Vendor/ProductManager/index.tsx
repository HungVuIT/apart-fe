...
function ProductManager() {
  ...
  const columns: GridColDef[] = [
    {
      ...
      renderCell: (params) => (
        <div className={classes["watch-name"]}>
          <img
            src={params.row.image[0] || defaultLogo}
            alt={params.row.name}
            className={classes.productAvt}
          />
        ...
        <div className={classes["price-box"]}>
          <div
            className={
              classes["watch-price"] +
              " " +
              (params.row.sale_off ? classes["old-price"] : "")
            }
          >
            {formatMoney.format(params.row.price)}
          </div>
          {params.row.sale_off && (
            <div className={classes["watch-price"]}>
              {formatMoney.format(params.row.sale_off.amount)}
            </div>
          )}
        </div>
      ),
    },
    {
      ...
      renderCell: (params) => (
        <div className={classes["watch-quantity"]}>
          <Button
            variant="contained"
            onClick={() => navigate(`/shop/watch/edit/${params.row.id}`)}
          >
            Sửa
          </Button>
        </div>
      ...
      renderCell: (params) => (
        <div className={classes["watch-action"]}>
          <Button
            variant="contained"
            color="error"
            onClick={async () => await handleDelProduct(params.row.id)}
          >
            Xóa
          </Button>
        </div>
  ...
  const handleChangeValue = (lst: any) => {
    ...
    switch (value) {
      case 0:
        ...
        break;
      case 1:
        result = lst.filter((item: any) => item.isHome);
        break;
      case 2:
        result = lst.filter((item: any) => !item.isHome);
        break;
  ...
  const productRender = useMemo(() => {
    const searchRender = lstProduct?.length
      ? [...lstProduct].filter((product: any) =>
          product.name
      ...
      <Box sx={{ width: "100%" }}>
        <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
          ...
          >
            ...
            <Tab label="Thiết kế" {...a11yProps(1)} />
            <Tab label="Bán lẻ" {...a11yProps(2)} />
          </Tabs>
        </Box>
